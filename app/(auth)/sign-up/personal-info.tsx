...
export default function PersonalInfo() {
  ...
  const checkDuplicate = async (email: string, phone: string) => {
    try {
      ...
      if (!response.ok) {
        console.error('❌ Duplicate check failed:', result);
        Alert.alert('Error', result.error || 'Failed to check account.');
        return false;
      }
      ...
      console.error('❌ Network error checking duplicate:', err);
      Alert.alert('Network Error', 'Unable to check account. Try again.');
      return false;
    }
  };
        ...
        >
          <AuthLayout withBackground>
            ...
            <View className="flex-1 w-full gap-4 mt-4">
              ...
              {/* Gender */}
              <View>
                <Text className="mb-1 ml-1 text-base font-medium text-foreground">
                  Gender
                </Text>
                <RadioGroup
                  value={gender}
                ...
                >
                  <View
                    className={`flex-row items-center gap-2 p-2 rounded-md ${
                      validationErrors.gender ? 'bg-red-50 border border-red-500' : ''
                    }`}
                  >
              ...
              {/* Agree */}
              <View
                className={`flex-row items-center mt-4 p-2 rounded-md ${
                  validationErrors.agree ? 'bg-red-50 border border-red-500' : ''
                }`}
              >
                <Checkbox
                  checked={agree}
                ...
                <Text className="ml-2 text-xs text-foreground">
                  ...
                  <Text className="text-[#8AA22F] font-semibold">
                    Terms and Condition
                  </Text>
                  , including the User{' '}
                  <Text className="text-[#8AA22F] font-semibold">
